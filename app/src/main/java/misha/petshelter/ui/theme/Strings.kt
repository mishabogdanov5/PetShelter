package misha.petshelter.ui.theme

const val EMAIL = "Email"
const val NAME = "Имя"
const val PASSWORD = "Пароль"
const val PASSWORD_AGAIN = "Повторите пароль"

const val SIGN_ON = "Регистрация"
const val SIGN_IN = "Вход"

const val SIGN_ON_BUTTON = "Зарегистрироваться"
const val SIGN_IN_BUTTON = "Войти"
const val SIGN_LATE = "Войти позже"
const val FORGOT_PASSWORD = "Забыли пароль?"

const val EMAIL_EXCEPTION = "Неверный формат email. Пример: name@gmail.com"
const val PASSWORD_EXCEPTION = "Поле не должно быть пустым!"

const val NAME_EXCEPTION = "Поле не должно быть пустым!"
const val PASSWORD_AGAIN_EXCEPTION = "Не совпадает с введенным паролем!"

const val EMAIL_PATTERN = "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@" +
        "[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$"

const val BASE_URL = "https://petsproject.issart.com/api/1.0.0/"

const val USER_PREFERENCES_NAME = "UserTokens"
const val TOKENS_NAME = "user_tokens"

const val SIGN_IN_EXCEPTION = "Ошибка входа"
const val SIGN_ON_EXCEPTION = "Ошибка регистрации"

const val AUTH_SCREEN = "AuthScreen"
const val MAIN_SCREEN = "MainScreen"

const val AUTH_GRAPH = "AuthGraph"
const val MAIN_GRAPH = "MainGraph"

const val ADVERTS = "Объявления"
const val CREATE_ADVERT = "Создать объявление"
const val PROFILE = "Профиль"